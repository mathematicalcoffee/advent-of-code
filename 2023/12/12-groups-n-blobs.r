...
# "group" = a run-length of springs we need to fit
#   (1,1,3 = 3 groups that are 1 1 and 3)
# for each blob
#   let max_n_groups be the max number of groups (from the front) that can fit
#   in the blob
...
process_record <- function(record) {
  return(
    make_record(
      record[[1]],
      as.numeric(stri_split_fixed(record[[2]], ",")[[1]])
    )
  )
}
...
n_ways <- function(groups, blobs, confirmed_springs_per_blob = NULL) {
  ...
  max_feasible_groups_in_blob <- max_n_groups_that_could_fit_in_blob(
    groups, length(blobs[[1]])
  )
  ...
  for (n_groups_to_put in seq(min_feasible_groups_in_blob, max_feasible_groups_in_blob)) {
    if (n_groups_to_put > 0) {
      # put n_groups_to_put into the first blob and multiply by the # of ways to
      #  put the rest in the rest of the blobs
      nnn <- n_ways_to_fit_groups_into_one_blob(
        groups[1:n_groups_to_put], blobs[[1]]
      )
      if (nnn > 0) {
        n <- n + nnn * n_ways(
          groups[-(1:n_groups_to_put)], blobs[-1], confirmed_springs_per_blob[-1]
        )
      }
    } else {
...
split_blob_at <- function(blob, first_blob_ends_at) {
  ...
  # returns: list of first new-blob, second new-blob, minus a character in the
  # middle for the dot - 1 because of the dot
  # **OR** no validation
...
n_ways_to_fit_groups_into_one_blob <- function(groups, blob) {
  ...
  if (length(groups) == 1) {
    ...
    max_feasible_blob_end <- length(blob) -
      min_chars_required_for(groups[-1], include_spacer = TRUE)
    # this code is very much like the n_ways_to_fit_one_group_into_one_blob,
    #  chould they be combined?
    ...
    if (length(spring_is)) {
      ...
      # there is probably something combinatorial that can be done here
    }
    ...
    while (first_blob_ends_at <= max_feasible_blob_end) {
      ...
      if (blob[first_blob_ends_at + 1] == "?") {
        ...
        n <- n +
          n_ways_to_fit_groups_into_one_blob(groups[-1], split_blobs[[2]])
      }
...
find_n_ways <- function(record) {
  return(n_ways(record$groups, record$blobs))
}
...
# 0.03    0.00    0.03
#
# I don't understand why forget() with global memoise is fine, but calling
#  memoise each loop for a fresh cache (the same thing) is not. it seems that
#  subsequent calls are /not/ memoised when called in this way.
# when typed in manually, re-memoising works; but in apply, it seems not to.
library(memoise)
...
q2 <- sapply(
  ...
  function(record) {
    forget(n_ways)
    forget(max_n_groups_that_could_fit_in_blob)
    forget(n_ways_to_fit_groups_into_one_blob)
    forget(n_ways_to_fit_one_group_into_one_blob)
    forget(split_blob_at)
    return(n_ways(record$groups, record$blobs))
  }
)

# Why the fuck is this one slow but the one above is fine
# f <- function (record) {
#   n_ways <- memoise(n_ways)
#   max_n_groups_that_could_fit_in_blob <- memoise(max_n_groups_that_could_fit_in_blob)
#   # on record 90 this is the one that hurts. it's one giant blob with many groups, unsure why memoisation is not working
#   n_ways_to_fit_groups_into_one_blob <- memoise(n_ways_to_fit_groups_into_one_blob)
#   n_ways_to_fit_one_group_into_one_blob <- memoise(n_ways_to_fit_one_group_into_one_blob)
#   split_blob_at <- memoise(split_blob_at)
#   return(n_ways(record$groups, record$blobs))
# }
# q2 <- sapply(records_simplified, f) # slow
